...
func _ready() -> void:
	monitoring = true
	monitorable = false
	# Assigned here rather than left to the scene, because a pickup is spawned
	# from a map marker that knows only an id — and the scene shipped with an
	# empty Sprite2D, which made every material in Orchardfall invisible.
	# Collectable and unseeable is worse than absent: the player concludes the
	# valley is empty and stops looking.
	if visual != null and visual.texture == null:
		var icon := Materials.icon_of(material_id)
		if icon == null:
