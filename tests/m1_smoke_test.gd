...
## The same rule, for the text that is *written* rather than coded.
##
## `_drawn_text_is_ascii` scans `ui/`, and the day it was added a screenshot of
## Maren's shop came back reading "Nobody decided it ù they just stopped." —
## from `resources/dialogue/shopkeeper.json`, which that check does not look at.
## Most of the words in this game are in `resources/`, not in `ui/`: every
## conversation, every item name, every quest summary. Checking only the code
## was checking the smaller half.
##
## Comment lines are skipped, and only quoted values are read. Half a dozen
## generated resources carry an em-dash in their `; Generated by ...` header and
## one `.tscn` has a `metadata/note` with a section sign in it — none of which
## is ever drawn, and none of which is worth rewriting to satisfy a test.
func _authored_text_is_ascii() -> void:
	var offenders: Array[String] = []
	var scanned := 0
	var files := _files_under("res://resources", [".tres", ".json"])
	if files.is_empty():
		# Source is not readable from an exported build, same as above.
		_check("the authored strings could be read", true, "no res://resources to scan")
		return
	for path in files:
		...
		while not handle.eof_reached():
			...
			for literal in _string_literals(line):
				...
				for i in literal.length():
					if literal.unicode_at(i) > 0x7E:
						offenders.append("%s:%d %s" % [path.get_file(), line_number,
							literal.substr(0, 32)])
						break
		handle.close()
	_check("every authored string is ASCII too, for the same reason",
		offenders.is_empty(), "; ".join(offenders))
	_check("and there were authored strings to check", scanned > 500,
		"%d literals in %d files" % [scanned, files.size()])


## Every file under `root` with one of these extensions, walking subfolders.
static func _files_under(root: String, suffixes: Array) -> Array[String]:
	...
	while not pending.is_empty():
		...
		for sub in dir.get_directories():
			pending.append("%s/%s" % [here, sub])
		for file in dir.get_files():
			for suffix in suffixes:
				if file.ends_with(String(suffix)):
					out.append("%s/%s" % [here, file])
					break
	out.sort()
	return out
